...
function updateSettingLabels() {
  ...
  if (el.labelSettingSfx)
    el.labelSettingSfx.innerText = `${Math.round(el.inputSettingSFX.value * 100)}%`;
  if (el.labelSettingShake)
    el.labelSettingShake.innerText = parseFloat(el.inputSettingShake.value).toFixed(1);
  if (el.labelSettingHaptics)
    el.labelSettingHaptics.innerText = parseFloat(el.inputSettingHapticsIntensity.value).toFixed(1);
  if (el.labelSettingUiScale)
    el.labelSettingUiScale.innerText = `${Math.round(el.inputSettingUiScale.value * 100)}%`;
  if (el.labelSettingFontScale)
    el.labelSettingFontScale.innerText = `${Math.round(el.inputSettingFontScale.value * 100)}%`;
  if (el.labelSettingDisplayScale)
    el.labelSettingDisplayScale.innerText = `${Math.round(el.inputSettingDisplayScale.value * 100)}%`;
}
...
[
  ...
  'input-setting-sfx',
  'input-setting-shake',
  'input-setting-haptics-intensity',
  'input-setting-ui-scale',
  'input-setting-font-scale',
  'input-setting-display-scale',
].forEach((id) => {
