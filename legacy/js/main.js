...
function changeState(newState, shouldAnimate = false) {
  ...
  const updateState = () => {
    ...
    [el.screenStart, el.screenClass, el.screenPlaying, el.screenTavern, el.screenEnd].forEach((s) =>
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Enter') {
    ...
    if (state.gameState === 'TAVERN') {
      if (
        ...
        document.activeElement.id !== 'btn-descend'
      ) {
  ...
  if (
    !el.modalDice.classList.contains('hide') ||
...
async function bootstrapGame() {
  ...
  if (config.displayScale)
    document.documentElement.style.setProperty('--display-scale', config.displayScale);
