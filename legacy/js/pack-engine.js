...
(function () {
  ...
  function _buildGameAPI(s) {
    ...
    return Object.freeze({
      get slides() {
        return stateRef.slidesTotalInEncounter || 0;
      },

      enemy: Object.freeze({
        get hp() {
          return stateRef.monsterHp;
        },
        get maxHp() {
          return stateRef.monsterMaxHp;
        },
        healHp(n) {
          stateRef.monsterHp = Math.min(stateRef.monsterMaxHp, stateRef.monsterHp + n);
        },
        dealDamage(n) {
          if (typeof applyDamage === 'function') applyDamage(n);
        },
      }),

      player: Object.freeze({
        get gold() {
          return stateRef.gold;
        },
        get multiplier() {
          return stateRef.multiplier;
        },
        get classId() {
          return stateRef.playerClass ? stateRef.playerClass.id : '';
        },
        addGold(n) {
          stateRef.gold += n;
        },
        drainSlides(n) {
          stateRef.slidesLeft = Math.max(0, stateRef.slidesLeft - n);
        },
        addMultiplier(n) {
          stateRef.multiplier += n;
        },
      }),

      spawnHazard(id) {
        const val =
          BUILTIN_HAZARDS[id] ?? (_customHazards[id] ? _customHazards[id].tileValue : null);
        if (val != null && typeof spawnRandomTile === 'function') spawnRandomTile(val);
      },

      get grid() {
        return (stateRef.grid || []).map((t) => (t ? { val: t.val } : null));
      },
      get gridArray() {
        return stateRef.grid || [];
      },
      clearTile(idx) {
        if (stateRef.grid && stateRef.grid[idx]) stateRef.grid[idx] = null;
      },
      setTile(idx, val) {
        if (stateRef.grid)
          stateRef.grid[idx] = {
            id: typeof tileIdCounter !== 'undefined' ? tileIdCounter++ : Date.now(),
            val,
            pop: true,
          };
      },
      ...
      playFx(name, r, c) {
        if (typeof playGridFx !== 'undefined') playGridFx(name, r, c);
      },
      shake() {
        if (typeof triggerScreenShake !== 'undefined') triggerScreenShake();
      },
      getArtifact(id) {
        return typeof getArtifactLevel !== 'undefined' ? getArtifactLevel(id) : 0;
      },
      addSlides(n) {
        stateRef.slidesLeft += n;
      },
      setHunterMark(n) {
        stateRef.hunterMarkLeft = n;
      },
      ...
      log(msg) {
        if (typeof addLog === 'function') addLog(String(msg));
      },

      shuffleTiles(n) {
        const filled = (stateRef.grid || []).map((t, i) => ({ t, i })).filter((x) => x.t);
        for (let i = 0; i < Math.min(n, filled.length); i++) {
          const a = _localPrngInt(0, filled.length - 1),
            b = _localPrngInt(0, filled.length - 1);
      ...
      degradeWeapon(sel) {
        let best = sel === 'best' ? 0 : Infinity,
          idx = -1;
        (stateRef.grid || []).forEach((t, i) => {
          if (!t || t.val <= 0) return;
          if ((sel === 'best' && t.val > best) || (sel === 'worst' && t.val < best)) {
            best = t.val;
            idx = i;
          }
        });
        if (idx >= 0) {
          stateRef.grid[idx].val = Math.max(2, stateRef.grid[idx].val / 2);
          addLog(`Enemy degraded your ${sel} weapon!`);
        }
      },
      ...
      destroyWeapon(sel) {
        ...
        if (idx >= 0) {
          stateRef.grid[idx] = null;
          addLog(`Enemy destroyed your ${sel} weapon!`);
        }
      },
  ...
  function _checkTrigger(ability, stateRef) {
    ...
    switch (ability.trigger) {
      case 'every_n_slides':
        return stateRef.slidesTotalInEncounter > 0 && stateRef.slidesTotalInEncounter % n === 0;
      case 'on_hp_below':
        return (stateRef.monsterHp / stateRef.monsterMaxHp) * 100 < n;
      case 'on_slide_start':
        return true;
      case 'on_damage':
        return false; // handled in onDamage
      case 'on_weapon_merge':
        return false; // handled in onMerge
      default:
        return false;
    }
  }
  ...
  function _applyEffect(ability, stateRef, G) {
    ...
    switch (ability.effect) {
      case 'spawn_hazard':
        ...
        break;
      case 'regen':
        stateRef.monsterHp = Math.min(stateRef.monsterMaxHp, stateRef.monsterHp + (p || 0));
        break;
      case 'tile_shuffle':
        G.shuffleTiles(p || 2);
        break;
      case 'weapon_degrade':
        G.degradeWeapon(p || 'best');
        break;
      case 'weapon_destroy':
        G.destroyWeapon(p || 'best');
        break;
      case 'drain_slides':
        G.player.drainSlides(p || 1);
        break;
      case 'drain_gold': {
      ...
      case 'crit_immune':
        break; // flagged on encounter, checked in d20.js
      case 'spell_cost_up':
        break; // flagged on encounter, checked in spells.js
    }
      ...
      const entry = {
        ...
        desc: (lvl) => {
          ...
          if (typeof art.desc === 'string') {
            try {
              if (art.desc.includes('${'))
                return new Function('lvl', `return \`${art.desc}\`;`)(lvl);
              return art.desc;
            } catch (e) {
              return art.desc;
            }
  ...
  function _runScript(scriptStr, stateRef, extraArgs) {
    ...
    try {
      ...
      const customKeys = Object.keys(extraArgs || {}).filter(
        (k) => !Object.keys(hardcoded).includes(k),
      );
  ...
  const PackEngine = {
    ...
    async applyActivePacks(stateRef) {
      ...
      Object.keys(CLASSES).forEach((k) => {
        if (_baseClassKeys && !_baseClassKeys.includes(k)) delete CLASSES[k];
      });
      ...
      for (const entry of packsToApply) {
        ...
        if (!pack) {
          if (entry.id === 'crit2048-default-enemies')
            pack =
              typeof CRIT2048_DEFAULT_ENEMIES_PACK !== 'undefined'
                ? CRIT2048_DEFAULT_ENEMIES_PACK
                : null;
          else if (entry.id === 'crit2048-default-classes')
            pack =
              typeof CRIT2048_DEFAULT_CLASSES_PACK !== 'undefined'
                ? CRIT2048_DEFAULT_CLASSES_PACK
                : null;
          else if (entry.id === 'crit2048-default-artifacts')
            pack =
              typeof CRIT2048_DEFAULT_ARTIFACTS_PACK !== 'undefined'
                ? CRIT2048_DEFAULT_ARTIFACTS_PACK
                : null;
          else if (entry.id === 'crit2048-default-weapons')
            pack =
              typeof CRIT2048_DEFAULT_WEAPONS_PACK !== 'undefined'
                ? CRIT2048_DEFAULT_WEAPONS_PACK
                : null;
          else if (entry.id === 'crit2048-default-hazards')
            pack =
              typeof CRIT2048_DEFAULT_HAZARDS_PACK !== 'undefined'
                ? CRIT2048_DEFAULT_HAZARDS_PACK
                : null;
          else if (entry.id === 'crit2048-default-skin')
            pack =
              typeof CRIT2048_DEFAULT_SKIN_PACK !== 'undefined' ? CRIT2048_DEFAULT_SKIN_PACK : null;
          else if (entry.id === 'crit2048-default')
            pack = typeof CRIT2048_DEFAULT_PACK !== 'undefined' ? CRIT2048_DEFAULT_PACK : null;
        }
      ...
      if (_activeSkin) _applySkin(_activeSkin, stateRef);
      else _resetSkin();

      console.log(
        `PackEngine: Applied ${packsToApply.length} pack(s). Encounters: ${ENCOUNTERS.length}`,
      );
    },
    ...
    validatePack(pack) {
      ...
      req.forEach((f) => {
        if (!pack[f]) errors.push(`Missing required field: "${f}"`);
      });
      if (pack.type && !VALID_TYPES.includes(pack.type))
        errors.push(`Invalid type: "${pack.type}"`);
      ...
      const checkScript = (script, ctx) => {
        ...
        Object.values(script).forEach((s) => {
          BLOCKED_KEYWORDS.forEach((kw) => {
            if (s.includes(kw)) errors.push(`${ctx}: blocked keyword "${kw}"`);
          });
        });
      };

      (pack.enemies || []).forEach((e, i) => {
        if (!e.id || !e.name || !e.hp || !e.slides || !e.mode)
          errors.push(`enemy[${i}]: missing required field`);
        if (e.mode === 'advanced') checkScript(e.script, `enemy "${e.id}"`);
      });
      (pack.classes || []).forEach((c, i) => {
        if (!c.id || !c.name || !c.icon || !c.desc)
          errors.push(`class[${i}]: missing required field`);
        if (c.ability?.mode === 'advanced')
          checkScript(c.ability.script, `class "${c.id}" ability`);
      });
      (pack.hazards || []).forEach((h, i) => {
        if (h.tileValue >= -7) errors.push(`hazard[${i}]: tileValue must be ≤ -8`);
      });
      (pack.artifacts || []).forEach((a, i) => {
        if (!a.id || !a.name || !a.icon || !a.desc)
          errors.push(`artifact[${i}]: missing required field`);
      });
      (pack.weapons || []).forEach((w, i) => {
        if ((w.tileValue === undefined && w.value === undefined) || !w.name || !w.icon)
          errors.push(`weapon[${i}]: missing required field`);
      });
    ...
    async isInstalled(id) {
      return await PackStorage.isInstalled(id);
    },
    getDamageReduction() {
      return _damageReduction;
    },
    getCustomHazard(id) {
      return _customHazards[id] || null;
    },
    ...
    onSlide(stateRef, direction) {
      ...
      if (enc.mode === 'simple') {
        ...
        if (stateRef.monsterHp > 0 && enc.passiveAbility && enc.passiveAbility.effect === 'regen') {
          stateRef.monsterHp = Math.min(
            stateRef.monsterMaxHp,
            stateRef.monsterHp + (enc.passiveAbility.effectParam || 0),
          );
        }
        // Passive damage_reduction — set flag for onDamage
        _damageReduction =
          enc.passiveAbility && enc.passiveAbility.effect === 'damage_reduction'
            ? enc.passiveAbility.effectParam || 0
            : 0;
    ...
    onDamage(stateRef, dmg) {
      ...
      if (
        enc.mode === 'simple' &&
        enc.primaryAbility &&
        enc.primaryAbility.trigger === 'on_damage'
      ) {
    ...
    onEncounterEnd(stateRef, enc) {
      ...
      if (stateRef.runStats)
        stateRef.runStats.customEnemiesDefeated =
          (stateRef.runStats.customEnemiesDefeated || 0) + 1;
    },
    ...
    onMerge(stateRef, newVal) {
      ...
      if (
        ...
        enc.mode === 'simple' &&
        enc.primaryAbility &&
        enc.primaryAbility.trigger === 'on_weapon_merge'
      ) {
      ...
      if (cls && cls._packDef) {
        const def = cls._packDef;
        if (
          def.passiveTrigger === 'on_merge' ||
          (def.passiveTrigger === 'on_merge_t3' && newVal >= 8)
        ) {
          this._dispatchSimpleEffect(def, stateRef);
        }
      }
    ...
    _dispatchSimpleEffect(def, stateRef, lvl = 1) {
      ...
      switch (def.passiveEffect) {
        case 'restore_slides':
          stateRef.slidesLeft += p;
          if (window.addLog) addLog(`${label}: +${p} slides!`);
          break;
        case 'add_gold':
          stateRef.gold += p;
          if (window.addLog) addLog(`${label}: +${p} gold!`);
          break;
        case 'add_multiplier':
          stateRef.multiplier += p;
          if (window.addLog) addLog(`${label}: +${p.toFixed(1)} Mult!`);
          break;
        case 'deal_damage':
          if (typeof applyDamage === 'function') applyDamage(p);
          break;
      }
    },
      ...
      if (stateRef.artifacts && typeof MASTER_ARTIFACTS !== 'undefined') {
        stateRef.artifacts.forEach((art) => {
          ...
          if (def && def.scripts && def.scripts.onMergeDamage) {
            ...
            _runScript(def.scripts.onMergeDamage, stateRef, {
              dmg: dmgObj,
              lvl: art.level,
              dir: direction,
              val: newVal,
    ...
    async getInstalledPacks() {
      ...
      if (typeof CRIT2048_SHADOWFELL_SKIN_PACK !== 'undefined')
        defaults.push(CRIT2048_SHADOWFELL_SKIN_PACK);
