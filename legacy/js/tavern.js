...
function renderTavern(skipAnimation = false) {
  ...
  if (respinBtn) {
    ...
    respinBtn.className =
      `shrink-0 px-3 py-1.5 rounded-lg font-black text-[10px] transition-all uppercase tracking-widest border ` +
      (canAffordRespin
        ? 'bg-slate-800 hover:bg-slate-700 text-amber-400 border-amber-600/30'
  ...
  const sortedPool = [...state.shopPool].sort(
  ...
  el.tavernArtifacts.innerHTML = sortedPool
    .map((art, index) => {
      ...
      let btnCls =
        `w-full px-4 py-3 rounded-2xl font-black text-[10px] transition-all uppercase tracking-widest border ` +
        (canAfford
      ...
      const shimmerHtml = hasShimmer
        ? '<div class="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/10 to-white/0 -translate-x-full animate-[shimmer_2s_infinite]"></div>'
        : '';
  ...
  if (state.playerClass && state.playerClass.ability) {
    ...
    if (el.btnRest) {
      ...
      el.btnRest.className =
        `py-4 font-bold rounded-2xl transition-all flex flex-col items-center justify-center gap-1 border transition-all duration-300 ` +
        (canRest
          ? 'bg-slate-950/40 hover:bg-emerald-950/20 text-emerald-400 border-slate-800 hover:border-emerald-500/30 group active:scale-95'
          : 'bg-slate-900/50 text-slate-600 border-slate-800 opacity-60 grayscale-[0.5] cursor-not-allowed');
    }

    if (el.btnUpgrade) {
      el.btnUpgrade.disabled = !canUpgrade;
      el.btnUpgrade.className =
        `py-4 font-bold rounded-2xl transition-all flex flex-col items-center justify-center gap-1 border transition-all duration-300 ` +
        (canUpgrade
          ? 'bg-slate-950/40 hover:bg-blue-950/20 text-blue-400 border-slate-800 hover:border-blue-500/30 group active:scale-95'
          : 'bg-slate-900/50 text-slate-600 border-slate-800 opacity-60 grayscale-[0.5] cursor-not-allowed');
    }

    if (el.btnAiOracle) {
      el.btnAiOracle.disabled = !canOracle;
      el.btnAiOracle.className =
        `w-full py-4 font-black rounded-2xl transition-all uppercase tracking-widest text-xs border shadow-lg transition-all duration-300 ` +
        (canOracle
          ? 'bg-indigo-600/90 hover:bg-indigo-500 text-white border-indigo-400/20 shadow-indigo-950/40 active:scale-95'
          : 'bg-slate-900/50 text-slate-600 border-slate-800 opacity-60 grayscale-[0.5] cursor-not-allowed shadow-none');
    }
  }

  el.btnDescend.innerText = ENCOUNTERS[state.encounterIdx + 1] ? `Next Ante` : `Final Boss`;
  renderHUD();
}
...
function nextEncounter() {
  ...
  overlay.className =
    'fixed inset-0 bg-slate-950 z-[150] flex flex-col items-center justify-center transition-all duration-1000 p-8';
  overlay.innerHTML = `
