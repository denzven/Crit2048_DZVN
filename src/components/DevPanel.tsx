...
const FX_PRESETS = [
  'stomp', 'flash', 'announce', 'aoe', 'lightning', 'projectile', 'smite', 'swirl', 'heal', 'poison', 'float', 'pop'
];

const DevPanel: React.FC = () => {
  const { 
    isDevMode, toggleDevMode, addGold, restoreSlides, 
    setMultiplier, setMonsterHp, monsterMaxHp, monsterHp,
    gold, slidesLeft, multiplier, activeArtifacts,
    buyArtifact, spawnRandomTile, nextEncounter, prevEncounter, 
    triggerFX, setD20Result, rollD20, executeDebugScript,
    setState
  } = useGameStore();
  ...
  const filteredArtifacts = Object.values(allArtifacts).filter(a => 
  ...
  return (
    ...
    >
        ...
        {isOpen && (
          ...
          >
            ...
            <section className="bg-slate-950 p-2 rounded-xl border border-slate-800">
              <h4 className="text-[10px] font-bold text-indigo-400 uppercase mb-2 px-1">Inject Script</h4>
              <textarea 
                value={debugScript}
            ...
            <div className="bg-slate-950/50 p-2 rounded-xl border border-slate-800 grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] font-mono">
              <div className="flex justify-between"><span className="text-slate-500">GOLD:</span> <span className="text-amber-400 font-bold">{gold}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">SLIDES:</span> <span className="text-emerald-400 font-bold">{slidesLeft}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">MULT:</span> <span className="text-purple-400 font-bold">x{multiplier.toFixed(2)}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">HP:</span> <span className="text-rose-400 font-bold">{Math.ceil(monsterHp)}</span></div>
            </div>

            {/* CORE CONTROLS */}
            <div className="grid grid-cols-2 gap-2">
              <DevButton label="+500 Gold" onClick={() => addGold(500)} color="bg-amber-600/20 text-amber-200 border-amber-600/30" />
              <DevButton label="+20 Slides" onClick={() => restoreSlides(20)} color="bg-emerald-600/20 text-emerald-200 border-emerald-600/30" />
              <DevButton label="Kill Boss" onClick={() => setMonsterHp(0)} color="bg-rose-600/20 text-rose-200 border-rose-600/30" />
              <DevButton label="Next Ante" onClick={() => nextEncounter()} color="bg-slate-800 border-slate-700" />
            </div>
            ...
            <section className="bg-slate-950 p-2 rounded-xl border border-slate-800">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                   <input 
                    type="range" min="0.1" max="10.0" step="0.1" value={multiplier} 
                    onChange={(e) => setMultiplier(parseFloat(e.target.value))}
                    className="flex-1 h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-purple-500"
                   />
                   <span className="text-[10px] font-bold text-purple-400 w-8">x{multiplier.toFixed(1)}</span>
                </div>
                <div className="grid grid-cols-4 gap-1">
                  <button onClick={() => forceD20(1)} className="text-[9px] bg-rose-900/40 text-rose-200 p-1 rounded border border-rose-900/50">NAT 1</button>
                  <button onClick={() => forceD20(10)} className="text-[9px] bg-slate-800 p-1 rounded border border-slate-700">SET 10</button>
                  <button onClick={() => forceD20(20)} className="text-[9px] bg-amber-900/40 text-amber-200 p-1 rounded border border-amber-900/50">NAT 20</button>
                  <button onClick={() => setState({ gameState: 'DICE' })} className="text-[9px] bg-indigo-600 text-white p-1 rounded font-bold">ROLL</button>
                </div>
              </div>
            </section>
            ...
            {/* VISUAL SANDBOX */}
            <section className="bg-slate-950 p-2 rounded-xl border border-slate-800">
              <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2 px-1">Visual Sandbox</h4>
              <div className="flex gap-2">
                ...
                >
                  {FX_PRESETS.map(fx => <option key={fx} value={fx}>{fx.toUpperCase()}</option>)}
                </select>
                <button 
                  ...
                  return (
                    <button
                      ...
                      className={`text-[9px] text-left px-2 py-1.5 rounded border transition-all flex justify-between items-center ${
                        owned ? 'bg-indigo-900/20 border-indigo-500/30 text-indigo-200' : 'bg-slate-800/50 border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                      }`}
                    >
                      ...
                      <span className="text-[8px] opacity-50 uppercase">{owned ? 'Owned' : 'Add'}</span>
                    </button>
                  );
                })}
              </div>
      ...
      >
        <span className={isOpen ? 'rotate-90' : 'rotate-0' + " transition-transform duration-300"}>🛠️</span>
      </button>
    </motion.div>
  );
};

const DevButton: React.FC<{ label: string; onClick: () => void; color: string }> = ({ label, onClick, color }) => (
  <button
    onClick={onClick}
