...
const GrimoireModal: React.FC<GrimoireProps> = ({ onClose, onEditPack }) => {
  ...
  const tabs = [
    { id: 'all', label: 'All' },
    ...Object.entries(packTypes).map(([key, def]) => ({ id: key, label: def.label }))
  ...
  const getPackTypeDef = (type: string) => packTypes[type] || { color: '#64748b', icon: '📦', label: type };
        ...
        <div className="p-4 border-b border-slate-800 flex justify-between items-center shrink-0 bg-slate-900/50 relative z-10">
          ...
          <div className="flex gap-2">
            <button 
              ...
              title="Refresh"
            >
              <motion.span animate={isFetching ? { rotate: 360 } : {}} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}>
                {isFetching ? '⏳' : '🔄'}
              </motion.span>
            </button>
            <button 
          ...
          {tabs.slice(0, 10).map(tab => (
            <button
              ...
              className={clsx(
                ...
                activeTab === tab.id ? "bg-violet-600 text-white shadow-lg border-violet-400/30" : "bg-slate-800 text-slate-400 hover:text-white border-slate-700"
              )}
            >
                ...
                return (
                  <motion.div
                    ...
                    className={clsx(
                      ...
                      isActive ? "border-violet-500/60 shadow-violet-900/10" : "border-slate-800 hover:border-slate-700"
                    )}
                    style={{ borderTopColor: typeDef.color, borderTopWidth: '2px' }}
                  >
                    {/* Type-colored top accent */}
                    <div className="absolute top-0 left-0 right-0 h-0.5 opacity-60" style={{ background: `linear-gradient(90deg, ${typeDef.color}, transparent)` }} />

                    <div className="absolute -top-4 -right-4 text-7xl opacity-5 group-hover:opacity-10 transition-opacity rotate-12 select-none pointer-events-none">{pack.icon}</div>
                    ...
                    <div className="flex gap-4 relative z-10">
                      ...
                      <div className="min-w-0 pr-12">
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <span className="px-2 py-0.5 text-[8px] font-black rounded uppercase tracking-tighter border" style={{ color: typeDef.color, borderColor: `${typeDef.color}40`, background: `${typeDef.color}10` }}>
                            {typeDef.icon} {typeDef.label}
                          </span>
                          {isActive && <span className="flex items-center gap-1 text-[8px] text-violet-400 font-black uppercase tracking-tighter"><span className="w-1.5 h-1.5 bg-violet-500 rounded-full animate-pulse" />Active</span>}
                        </div>
                      </div>
                    </div>
                      ...
                      {isInstalled ? (
                        <>
                          {(pack as any).isTemplate ? (
                            ...
                            <button 
                              ...
                              className={clsx(
                                ...
                                isActive ? "bg-violet-900/20 text-violet-400 border-violet-500/30 hover:bg-violet-900/40" : "bg-slate-800 hover:bg-slate-700 text-white border-slate-700 shadow-lg shadow-slate-950"
                              )}
                            >
                          ...
                          <div className={clsx("grid gap-2", (pack as any).isTemplate ? "grid-cols-1" : "grid-cols-3")}>
                            ...
                            <button onClick={() => handleDuplicate(pack.id)} className="py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[8px] font-black uppercase tracking-widest border border-slate-700">Clone</button>
                            {!(pack as any).isTemplate && (
          ...
          {filteredPacks.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-slate-500 gap-4">
              ...
              <p className="text-xs font-black uppercase tracking-widest opacity-50">
                {searchQuery ? `No packs matching "${searchQuery}"` : 'No packs found in this archive'}
              </p>
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="text-[10px] text-violet-400 font-black uppercase tracking-widest hover:underline">Clear Search</button>
              )}
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2 bg-slate-900 border-t border-slate-800 shrink-0 flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-slate-500 relative z-10">
          <div className="flex items-center gap-4">
            <span className={clsx("flex items-center gap-1.5", isOnline ? "text-emerald-400" : "text-rose-400")}>
              <span className={clsx("w-2 h-2 rounded-full", isOnline ? "bg-emerald-500 animate-pulse" : "bg-rose-500")} /> 
