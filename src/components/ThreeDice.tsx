      ...
      if (!ending) {
          ...
          for (let j = i + 1; j < diceObjects.length; j++) {
            ...
            if (dist < 2.0) {
              ...
              if (velocityAlongNormal < 0) {
                ...
                d2.userData.avel.addScalar((Math.random() - 0.5) * 0.5);
              }
            }
          }
        }
        diceObjects.forEach((m) => {
          ...
          if (m.position.y <= 0.9) {
            ...
            vel.x *= 0.6;
            vel.z *= 0.6;
            avel.multiplyScalar(0.6);
          }
          if (frames > 35) m.quaternion.slerp(targetQuat, 0.1);
          if (Math.abs(m.position.x) > spreadX) {
            m.position.x = Math.sign(m.position.x) * spreadX;
            vel.x *= -0.7;
          }
          if (Math.abs(m.position.z) > spreadZ) {
            m.position.z = Math.sign(m.position.z) * spreadZ;
            vel.z *= -0.7;
          }
          if (Math.abs(vel.y) > 0.05 || vel.lengthSq() > 0.005 || avel.lengthSq() > 0.05)
            moving = true;
        });
        if (!moving && frames > 65) {
          ending = true;
          diceObjects.forEach((m) => {
            m.userData.finalPos = m.position.clone();
          });
