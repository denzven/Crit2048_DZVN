...
const ArtifactTriggerOverlay: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[1000] overflow-hidden">
      <AnimatePresence>
        {activeFX.filter(fx => fx.params?.targetId || (fx.params?.x === undefined && fx.params?.y === undefined)).map((fx) => {
          ...
          return (
            <motion.div
              key={fx.id}
              initial={fx.name === 'stomp' ? { scale: 3, opacity: 0, y: isPositional ? -20 : -200 } : { scale: 0, opacity: 0, y: 20 }}
              animate={fx.name === 'stomp' ? { scale: 1, opacity: 1, y: 0 } : { scale: 1.5, opacity: 1, y: -80 }}
              exit={{ scale: 0, opacity: 0, transition: { duration: 0.2 } }}
              transition={fx.name === 'stomp' 
