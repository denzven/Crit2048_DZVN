...
const DiceModal: React.FC = () => {
  ...
  useEffect(() => {
    if (
      ...
      lastRoll &&
      currentModIdx >= 0 &&
      currentModIdx < lastRoll.modifiers.length
    ) {
    ...
    <div id="modal-dice" className="absolute inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <AnimatePresence>
          {tier && stage !== 'idle' && (
            <motion.div
              ...
              style={{
                background: `radial-gradient(ellipse at 50% 40%, ${tier.bgColor} 0%, transparent 70%)`,
              }}
            />
          )}
        </AnimatePresence>

        <h2 className="text-2xl font-black mb-1 text-rose-500 uppercase tracking-widest mt-2 font-serif relative z-10">
          D20 Check
        </h2>
        <p className="text-slate-400 text-xs uppercase tracking-widest mb-6 font-bold relative z-10">
          Base Mod:{' '}
          <span className="text-white bg-slate-800 px-2 py-0.5 rounded ml-1">
        ...
        <div className="relative w-full h-64 bg-slate-950 rounded-2xl border border-slate-800 my-2 flex flex-col items-center justify-center overflow-hidden">
          <div
            id="d20-render-target"
            className="w-full h-full absolute inset-0 mx-auto z-10 flex items-center justify-center"
          >
            {showThreeDice && (
              <ThreeDice sides={20} results={diceResults} onComplete={onDiceComplete} />
            )}
          </div>
              ...
              >
                ...
                <div className="h-8 mb-2 flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    {stage === 'modifying' &&
                      currentModIdx >= 0 &&
                      lastRoll?.modifiers &&
                      lastRoll.modifiers[currentModIdx] && (
                        <motion.div
                        ...
                        >
                          ...
                          <span className="text-xs text-rose-400 font-bold">
                            {lastRoll.modifiers[currentModIdx].label}
                          </span>
                          <span className="text-xs text-white font-mono">
                            {lastRoll.modifiers[currentModIdx].type === 'add'
                              ? `+${lastRoll.modifiers[currentModIdx].val}`
                              : lastRoll.modifiers[currentModIdx].type === 'multiply'
                                ? `x${lastRoll.modifiers[currentModIdx].val}`
                                : `→ ${lastRoll.modifiers[currentModIdx].val}`}
                          </span>
                        </motion.div>
                      )}
                ...
                {stage === 'finalized' && tier && (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400 font-medium">{tier.sublabel}</span>
                  </motion.div>
                )}
              </motion.div>
            )}
