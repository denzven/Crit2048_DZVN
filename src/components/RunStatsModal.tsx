...
const RunStatsModal: React.FC<{ onShowLeaderboard: () => void, onShowShare: () => void }> = ({ onShowLeaderboard, onShowShare }) => {
  const { gameState, runStats, playerClass, encounterIdx, artifacts, resetGame, isChallengeMode, rivalData, score, showAlert } = useGameStore();
  ...
  const handleChallengeShare = () => {
    ...
    if (navigator.share) {
      ...
      SFX.coin();
      showAlert("Challenge Ready!", "Your unique challenge link has been copied to the clipboard. Send it to a friend to start the rivalry!");
    }
  };
  ...
  const handleResultShare = () => {
    ...
    if (navigator.share) {
      ...
      showAlert("Result Copied!", "Your victory status and re-challenge link have been copied. Go show them who's boss!");
    }
  };
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 px-2 py-1.5 bg-white/5 rounded-xl border border-white/5">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[7px] text-slate-500 font-black uppercase tracking-widest">You</span>
                </div>
                <div className="text-[10px] text-slate-600 font-black italic">VS</div>
                <div className="flex flex-col items-center">
                  <span className="text-lg">{rivalData.icon || '👤'}</span>
                  <span className="text-[7px] text-slate-500 font-black uppercase tracking-widest">{rivalData.name || 'Rival'}</span>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <StatCompareRow label="Max DMG" you={runStats.maxDamage} them={rivalData.maxDamage} />
                <StatCompareRow label="Total DMG" you={runStats.totalDamageDealt} them={rivalData.damage} />
                  ...
                  {isChallengeMode ? (
                    <button 
                      ...
                      className={clsx(
                        ...
                        didWinChallenge ? "bg-emerald-600 border-emerald-500/30" : "bg-rose-600 border-rose-500/30"
                      )}
                    >
...
const StatCompareRow: React.FC<{ label: string, you: number, them: number }> = ({ label, you, them }) => {
  const win = you >= them;
  return (
    <div className="flex justify-between items-center text-[10px]">
      <span className="text-slate-500 uppercase font-bold text-[8px]">{label}</span>
      <div className="flex items-center gap-2 font-mono">
        <span className={clsx("font-black", win ? "text-emerald-400" : "text-slate-300")}>{you}</span>
