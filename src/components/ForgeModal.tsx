    ...
    try {
      ...
      if (store) {
        ...
        if (!activeIds.includes(pack.id)) {
          store.setState({
            runStats: { ...store.runStats, activePackIds: [...activeIds, pack.id] },
          });
        }
      ...
      >
        ...
        <div className="p-4 border-b border-slate-800 flex justify-between items-center shrink-0 bg-slate-900/80 backdrop-blur-md z-10">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 flex items-center justify-center bg-slate-800 hover:bg-rose-900 text-slate-300 rounded-lg border border-slate-700 transition-colors"
            >
              ✕
            </button>
          </div>
                ...
                <span
                  className={clsx(
                    'w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black border transition-all shrink-0',
                      ...
                      ? 'bg-emerald-500 border-emerald-400 text-white animate-check-pop'
                      : active
            ...
            {step === 1 && (
              <div className="space-y-6">
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
                  <h3 className="text-white font-black uppercase tracking-widest text-xs border-b border-slate-800 pb-3">
                    Pack Identification
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        value={pack.id}
                        onChange={(e) =>
                          updatePack({ id: e.target.value.toLowerCase().replace(/\s/g, '-') })
                        }
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {contentTypes.map((ct) => (
                    <button
                      ...
                      style={
                        activeSubTab === ct.key
                          ? { background: ct.color, borderColor: `${ct.color}80` }
                          : {}
                      }
                    >
                      {ct.icon} {ct.label} (
                      {Array.isArray((pack as any)[ct.key])
                        ? ((pack as any)[ct.key] as any[]).length
                        : 0}
                      )
                    </button>
                  ))}
                ...
                <div className="space-y-4">
                  {editingIndex !== null &&
                  (pack[activeSubTab as keyof PackData] as any[])?.[editingIndex] ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                          <div className="flex items-center gap-4">
                            <h4 className="text-white font-black uppercase tracking-widest text-sm">
                              Editing {activeSubTab.slice(0, -1)}
                            </h4>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="space-y-2">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const list = [
                                      ...(pack[activeSubTab as keyof PackData] as any[]),
                                    ];
                                    list[editingIndex] = {
                                      ...list[editingIndex],
                                      icon: e.target.value,
                            ...
                            {activeSubTab === 'heroes' && (
                              <div className="space-y-6">
                                ...
                                <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 space-y-4">
                                  <h5 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] border-b border-slate-800 pb-2">
                                    Active Ability (Spell)
                                  </h5>
                                  <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1">
                                      <label className="text-[8px] font-black text-slate-700 uppercase">
                                        Name
                                      </label>
                                      <input
                                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-2 text-white text-[10px]"
                                        value={
                                          (pack.heroes as any[])[editingIndex].ability?.name || ''
                                        }
                                        onChange={(e) => {
                                          const list = [...(pack.heroes || [])];
                                          list[editingIndex] = {
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-2 text-white text-[10px]"
                                        value={
                                          (pack.heroes as any[])[editingIndex].ability?.sides || 6
                                        }
                                        onChange={(e) => {
                                          const list = [...(pack.heroes || [])];
                                          list[editingIndex] = {
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-2 text-white text-[10px]"
                                        value={
                                          (pack.heroes as any[])[editingIndex].ability?.maxUses || 1
                                        }
                                        onChange={(e) => {
                                          const list = [...(pack.heroes || [])];
                                          list[editingIndex] = {
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <select
                                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-2 text-white text-[10px]"
                                        value={
                                          (pack.heroes as any[])[editingIndex].ability?.type ||
                                          'damage'
                                        }
                                        onChange={(e) => {
                                          const list = [...(pack.heroes || [])];
                                ...
                                {(pack.monsters as any[])[editingIndex].mode === 'simple' ? (
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <textarea
                                      ...
                                      spellCheck={false}
                                      value={JSON.stringify(
                                        (pack.monsters as any[])[editingIndex].script || {},
                                        null,
                                        2,
                                      )}
                                      onChange={(e) => {
                            ...
                            <div className="space-y-2">
                              ...
                              <textarea
                                ...
                                value={
                                  (pack[activeSubTab as keyof PackData] as any[])[editingIndex]
                                    .lore ||
                                  (pack[activeSubTab as keyof PackData] as any[])[editingIndex].desc
                                }
                                onChange={(e) => {
                                  const list = [...(pack[activeSubTab as keyof PackData] as any[])];
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-800">
                              ...
                              {logicMode === 'visual' ? (
                                <div className="space-y-3">
                                  ...
                                  {hookDefs.map((hook) => {
                                    const current = (pack[activeSubTab as keyof PackData] as any[])[
                                      editingIndex
                                    ];
                                    const scripts = current.passiveTriggers || {};
                                    const script = scripts[hook.id]
                                      ? JSON.stringify(scripts[hook.id], null, 2)
                                      : '';

                                    return (
                                      <div
                                        key={hook.id}
                                        className="bg-slate-950/50 border border-slate-800/50 rounded-xl p-3 space-y-2"
                                      >
                                        <div className="flex justify-between items-center">
                                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                            <span className="text-xs">{hook.icon}</span>{' '}
                                            {hook.label}
                                          </span>
                                          {script ? (
                                            <button
                                              onClick={() => {
                                                const list = [
                                                  ...(pack[
                                                    activeSubTab as keyof PackData
                                                  ] as any[]),
                                                ];
                                              ...
                                              onChange={(e) => {
                                                ...
                                                if (e.target.value === 'gold')
                                                  actions = [
                                                    ...
                                                    { type: 'log', stringParam: 'Gold found!' },
                                                  ];
                                                if (e.target.value === 'hazard')
                                                  actions = [
                                                    { type: 'spawn_hazard', amount: -1 },
                                                    {
                                                      type: 'log',
                                                      stringParam: 'Something spawned...',
                                                    },
                                                  ];
                                                if (e.target.value === 'heal')
                                                  actions = [
                                                    { type: 'regen', target: 'player', amount: 2 },
                                                    { type: 'log', stringParam: 'Life restored!' },
                                                  ];
                                                if (e.target.value === 'damage')
                                                  actions = [
                                                    ...
                                                    { type: 'log', stringParam: 'Direct strike!' },
                                                  ];
                                        ...
                                        {script && (
                                          <div className="bg-slate-900/50 rounded-lg px-3 py-2 border border-rose-500/10">
                                            <p className="text-[9px] text-rose-400/80 font-mono line-clamp-2">
                                              {script}
                                            </p>
                                          </div>
                                        )}
                      ...
                      <div className="space-y-6">
                        <div className="bg-slate-950/50 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col items-center justify-center min-h-[400px] relative overflow-hidden group">
                          ...
                          <h5 className="absolute top-6 left-6 text-[9px] font-black text-slate-700 uppercase tracking-[0.3em]">
                            Live Simulation
                          </h5>
                              ...
                              <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="text-center">
                                  <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic">
                                    {(pack.monsters as any[])[editingIndex].name}
                                  </h3>
                                  <p className="text-[10px] text-rose-500 font-black uppercase tracking-widest">
                                    {(pack.monsters as any[])[editingIndex].hp} HP /{' '}
                                    {(pack.monsters as any[])[editingIndex].slides} Slides
                                  </p>
                                </div>
                              </div>
                              <div className="space-y-2">
                                ...
                                <p className="text-center text-[9px] text-slate-500 font-medium italic">
                                  "{(pack.monsters as any[])[editingIndex].lore}"
                                </p>
                              </div>
                            </div>
                          )}
                          ...
                          {activeSubTab === 'artifacts' && (
                            <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-500">
                              <div className="w-32 h-32 bg-slate-900 rounded-[2rem] flex items-center justify-center text-6xl shadow-2xl border border-slate-800 relative">
                                <div className="absolute -top-2 -right-2 bg-rose-600 text-white text-[8px] font-black px-2 py-1 rounded-lg">
                                  {(pack.artifacts as any[])[editingIndex].rarity}
                                </div>
                                {(pack.artifacts as any[])[editingIndex].icon}
                              </div>
                              <div className="text-center space-y-2">
                                <h4 className="text-xl font-black text-white">
                                  {(pack.artifacts as any[])[editingIndex].name}
                                </h4>
                                <p className="text-[10px] text-slate-400 max-w-[200px] leading-relaxed font-medium italic">
                                  {(pack.artifacts as any[])[editingIndex].desc}
                                </p>
                                <p className="text-amber-500 font-mono font-black">
                                  💰{(pack.artifacts as any[])[editingIndex].basePrice}
                                </p>
                              </div>
                            </div>
                          )}
                            ...
                            <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-500">
                              ...
                              <div className="text-center">
                                <h4 className="text-lg font-black text-white">
                                  {(pack.arsenal as any[])[editingIndex].name}
                                </h4>
                                <div className="mt-2 inline-flex items-center gap-2 bg-rose-500/20 text-rose-500 px-3 py-1 rounded-full border border-rose-500/30">
                                  ...
                                  <span className="font-mono font-black">
                                    {(pack.arsenal as any[])[editingIndex].dmg}
                                  </span>
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                          <h6 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">
                            Integrity Report
                          </h6>
                          <div className="space-y-3">
                            {!(pack[activeSubTab as keyof PackData] as any[])[editingIndex].id && (
                              <div className="flex items-center gap-3 text-amber-500 text-[10px] font-bold bg-amber-500/5 p-3 rounded-xl border border-amber-500/20">
                                <span>⚠️</span>
                                <span>
                                  Internal ID is missing. Entry will be ignored during export.
                                </span>
                              </div>
                            )}
                            {!(pack[activeSubTab as keyof PackData] as any[])[editingIndex]
                    ...
                    <>
                      {(pack[activeSubTab as keyof PackData] as any[])?.map(
                        (item: any, idx: number) => (
                          <div
                            key={idx}
                            onClick={() => setEditingIndex(idx)}
                            className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 flex gap-4 items-center group cursor-pointer hover:bg-slate-800 transition-colors"
                          >
                            <div className="w-12 h-12 bg-slate-950 rounded-lg flex items-center justify-center text-2xl border border-slate-800 shrink-0">
                              {item.icon || '❓'}
                            </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          const newEntry =
                            activeSubTab === 'monsters'
                              ? {
                                  id: `new-${activeSubTab}-${list.length}`,
                                  name: `New Monster`,
                                  icon: '👹',
                                  hp: 100,
                                  slides: 20,
                                  lore: 'Description here',
                                  mode: 'simple',
                                }
                              : {
                                  id: `new-${activeSubTab}-${list.length}`,
            ...
            {step === 4 && (
              <div className="text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 uppercase font-bold mt-2 tracking-widest">
                    Verify your pack data before sealing the grimoire
                  </p>
                </div>
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 text-left space-y-2">
                  ...
                  <div className="flex gap-4 flex-wrap pt-1">
                    {contentTypes
                      .filter((ct) => ((pack as any)[ct.key] || []).length > 0)
                      .map((ct) => (
                        <div
                          key={ct.key}
                        ...
                        >
                          <span>{ct.icon}</span>
                          <span style={{ color: ct.color }}>
                            {((pack as any)[ct.key] || []).length}
                          </span>
            ...
            {steps.map((s) => (
              <div
                ...
                className={clsx(
                  ...
                  step === s.id ? 'bg-indigo-500 w-4' : 'bg-slate-700',
                )}
              ></div>
            ))}
          </div>
