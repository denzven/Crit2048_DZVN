          ...
          {step === 'confirm' ? (
            ...
            >
              ...
              <p className="text-slate-400 text-xs mb-8 leading-relaxed">
                ...
                <span className="text-indigo-400 font-bold">Leaderboards will be disabled</span> for
                this session.
              </p>
            ...
            >
              <h3 className="text-indigo-400 font-black text-[10px] uppercase tracking-[0.3em] mb-4">
                Security Protocol
              </h3>

              <div
                className={`w-full bg-slate-950 rounded-xl p-4 mb-6 border-2 transition-colors ${error ? 'border-rose-500 animate-shake' : 'border-slate-800'}`}
              >
                <div className="flex justify-center gap-2 h-6">
                  {Array(5)
                    ...
                    .map((_, i) => (
                      <div
                        key={i}
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${pin.length > i ? 'bg-indigo-500 scale-110' : 'bg-slate-800'}`}
                      />
                    ))}
                </div>
              </div>
