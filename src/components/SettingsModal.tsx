    ...
    <div className="absolute inset-0 z-[160] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          <section>
            <h3 className="text-rose-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
              Dungeon Rules
            </h3>
            <div className="space-y-4">
              <div>
                <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                  Moves per Roll <span className="text-white">{settings.movesPerRoll}</span>
                </label>
                <input
                  type="range"
                  min="1"
              ...
              <div>
                <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                  Starting Gold <span className="text-amber-400">{settings.startingGold}</span>
                </label>
                <input
                  type="range"
                  min="0"
              ...
              <div className="pt-2">
                <label className="block text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                  Fixed Run Seed
                </label>
                <input
                  type="text"
                ...
                />
                <p className="text-[8px] text-slate-600 mt-1.5 italic">
                  Leave blank for a unique random seed every descent.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="pt-6 border-t border-slate-800">
            <h3 className="text-blue-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
              Atmosphere
            </h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateSettings({
                      diceTheme: e.target.value as 'default' | 'neon' | 'wood' | 'stone',
                    })
                  }
              ...
              {settings.screenshake && (
                <div>
                  <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                    Shake Intensity{' '}
                    <span className="text-white">{settings.shakeIntensity.toFixed(1)}</span>
                  </label>
                  <input
                    type="range"
                    min="0"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                    UI Scale <span className="text-white">{settings.uiScale.toFixed(1)}x</span>
                  </label>
                  <input
                    type="range"
                    min="0.8"
                ...
                <div>
                  <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                    Font Scale <span className="text-white">{settings.fontScale.toFixed(1)}x</span>
                  </label>
                  <input
                    type="range"
                    min="0.8"
          ...
          <section className="pt-6 border-t border-slate-800">
            <h3 className="text-emerald-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
              Sensory
            </h3>
            <div className="space-y-4">
              <div>
              ...
              {settings.haptics && (
                <>
                  <div>
                    <label className="flex justify-between text-slate-400 text-[10px] mb-2 font-bold uppercase tracking-wider">
                      Haptic Intensity{' '}
                      <span className="text-white">{settings.hapticIntensity.toFixed(1)}</span>
                    </label>
                    <input
                      type="range"
                      min="0"
                      ...
                      onChange={(e) =>
                        updateSettings({ hapticIntensity: parseFloat(e.target.value) })
                      }
                      className="w-full accent-emerald-500"
                    />
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    <button
                      onClick={() => Native.vibrate(20, settings.hapticIntensity)}
                      className="py-2 bg-slate-800 text-slate-400 text-[8px] font-black uppercase rounded-lg border border-slate-700 hover:bg-slate-700 transition-colors"
                    >
                      Light
                    </button>
                    <button
                      onClick={() => Native.vibrate(50, settings.hapticIntensity)}
                      className="py-2 bg-slate-800 text-slate-400 text-[8px] font-black uppercase rounded-lg border border-slate-700 hover:bg-slate-700 transition-colors"
                    >
                      Med
                    </button>
                    <button
                      onClick={() => Native.vibrate(100, settings.hapticIntensity)}
                      className="py-2 bg-slate-800 text-slate-400 text-[8px] font-black uppercase rounded-lg border border-slate-700 hover:bg-slate-700 transition-colors"
                    >
                      Heavy
                    </button>
                    <button
                      onClick={() => Native.vibrate([100, 50, 100], settings.hapticIntensity)}
                      className="py-2 bg-slate-800 text-slate-400 text-[8px] font-black uppercase rounded-lg border border-slate-700 hover:bg-slate-700 transition-colors"
                    >
                      Long
                    </button>
          ...
          <button
            ...
            className="w-full py-3 text-slate-500 hover:text-rose-400 transition-colors text-[9px] font-black uppercase tracking-widest border border-dashed border-slate-800 rounded-xl"
          >
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(
                    'https://api.github.com/repos/denzven/Crit2048-DZVN/commits/main',
                  );
        ...
        <div className="p-6 border-t border-slate-800 flex flex-col gap-3 shrink-0">
            ...
            <button
              onClick={() => {
                showConfirm(
                  ...
                  () => {
                    useGameStore.getState().forfeitRun();
                    onClose();
                  },
                );
              }}
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 py-3 bg-slate-800 text-white font-black rounded-xl hover:bg-slate-700 transition-colors uppercase tracking-widest text-[10px] border border-slate-700"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 py-3 bg-rose-600 text-white font-black rounded-xl hover:bg-rose-500 transition-colors uppercase tracking-widest text-[10px] border border-rose-500/50 shadow-lg"
            >
              Save Config
            </button>
          </div>
