...
const Tavern: React.FC = () => {
  ...
  const respinTavern = React.useCallback(() => {
  ...
  const handleAction = React.useCallback(() => {
    if (focusedIndex === 0) restoreSpells();
      ...
      if (gold >= (svc?.oracle?.cost ?? 50)) {
        ...
        const legendaries = activeArtifacts.filter(
          (a) => a.rarity?.toLowerCase() === 'legendary' || a.rarity?.toLowerCase() === 'artifact',
        );
        if (legendaries.length > 0) {
          ...
          handleBuyArtifact(pick.id);
          addLog(`Oracle: The legendary ${pick.name} has been bestowed!`);
        } else {
          addLog(
            svc?.oracle?.emptyMessage ?? 'Oracle: No legendary artifacts found in this realm...',
          );
        }
      }
  ...
  }, [focusedIndex, shopItems, gold, totalOptions, handleAction]);
  ...
  return (
    <div id="screen-tavern" className="w-full flex flex-col h-full relative z-10 px-4">
      <div
        id="tavern-scroll-area"
        className="flex-grow overflow-y-auto pb-6 space-y-6 flex flex-col min-h-0 custom-scrollbar"
      >
        <div className="tavern-content max-w-2xl mx-auto w-full">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-black text-amber-500 mb-1 font-serif tracking-tight">
              Tavern
            </h2>
            <p className="text-slate-500 text-[10px] uppercase tracking-[0.3em] font-bold">
              spend gold to buy or level up artifacts
            </p>
          </motion.div>
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-4 md:p-5 shadow-2xl backdrop-blur-xl">
              ...
              <div className="grid grid-cols-2 gap-3">
                  ...
                  title={
                    gold < restCost
                      ...
                      : usesLeft >= (playerClass?.ability?.maxUses || 0)
                        ? (svc?.rest?.disabledReason ?? 'Already at max')
                        : (svc?.rest?.description ?? 'Restore spell uses')
                  }
                  className={clsx(
                ...
                <button
                  ...
                  title={
                    gold < upgradeCost
                      ? `Need ${upgradeCost} gold`
                      : !playerClass?.ability
                        ? (svc?.enhance?.disabledReason ?? 'No spell')
                        : (svc?.enhance?.description ?? 'Upgrade spell')
                  }
                  className={clsx(
                    ...
                    if (gold >= oracleCost) {
                      ...
                      const legendaries = activeArtifacts.filter(
                        (a) =>
                          ...
                          a.rarity?.toLowerCase() === 'artifact',
                      );
                      if (legendaries.length > 0) {
                        useGameStore.setState({ gold: gold - oracleCost });
                        const pick =
                          legendaries[Math.floor(SeededRNG.random() * legendaries.length)];
                        handleBuyArtifact(pick.id);
                        addLog(`Oracle: The legendary ${pick.name} has been bestowed!`);
                      } else {
                        addLog(
                          svc?.oracle?.emptyMessage ??
                            'Oracle: No legendary artifacts found in this realm...',
                        );
                      }
                    }
                  }}
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between shrink-0 px-2">
              <h3 className="text-slate-500 font-black uppercase tracking-[0.2em] text-[10px]">
                Merchant's Collection
              </h3>
              <motion.button
                ...
                return (
                  <motion.div
                    ...
                    className={clsx(
                      ...
                      focusedIndex === 4 + idx
                        ? 'border-white/30 shadow-amber-950/20'
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center z-10">
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        <span
                          className={clsx(
                            'text-[11px] uppercase font-black border-b-2 pb-0.5 tracking-widest',
                            styles.color,
                            styles.border,
                          )}
                        >
                    ...
                    <div className="flex-grow flex flex-col z-10">
                      ...
                      <div className="flex flex-col gap-4 mt-auto">
                        <div className="flex justify-between items-center bg-slate-950/60 rounded-2xl px-4 py-3 border border-slate-800/50 shadow-inner">
                          ...
                          <span
                            className={clsx(
                              'font-mono text-sm font-black flex items-center gap-1.5',
                              gold >= currentCost ? 'text-amber-400' : 'text-rose-500',
                            )}
                          >
      ...
      <div className="pt-4 pb-8 text-center shrink-0 w-full max-w-2xl mx-auto">
        <button
          ...
          className={clsx(
            ...
            focusedIndex === totalOptions
              ? 'border-white ring-4 ring-rose-500/40 scale-105'
              : 'border-rose-500/30',
          )}
        >
          Continue
